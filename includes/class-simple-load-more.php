...
class Simple_Load_More {

	/**
	 * The single instance of Simple_Load_More.
	 * @var 	object
	 * @access  private
	 * @since 	1.0.0
	 */
	...
	public function __construct ( $file = '', $version = '1.0.0' ) {
		...
		if ( is_admin() ) {
			$this->admin = new Simple_Load_More_Admin_API();
		}
	...
	public function load_localisation () {
		load_plugin_textdomain( 'simple-load-more', false, dirname( plugin_basename( $this->file ) ) . '/lang/' );
	...
	/**
	 * Main Simple_Load_More Instance
	 *
	 * Ensures only one instance of Simple_Load_More is loaded or can be loaded.
	 *
	 * @since 1.0.0
	 * @static
	 * @see Simple_Load_More()
	 * @return Main Simple_Load_More instance
	 */
	 ...
	 * @return  string
	 */
	public function build_load_more_button($loading_icon,$button_text,$loading_text,$animation_open) {
		global $wp_query;
		global $wp_rewrite;
		...
		$animation_open = get_option('elm_animation_open');*/
		 $cat_taxonomy  =   get_queried_object()->taxonomy;
		...
		if($animation_open == 'on'){
		$html .= $this->build_loading_icon($loading_icon);
		}else{
		$html .= '    <div class="elm-loading-anim">'.$loading_text.'</div>' . "\n";
		}
